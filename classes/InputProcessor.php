...
class InputProcessor {
    ...
    public static function processEmail(string $email): array {
        ...
        if (empty($email)) {
            return self::returnInput("Please enter an email.", false);
        }
        //Sanitize
        ...
        if (filter_var($value, FILTER_VALIDATE_EMAIL)) {
            ...
            return self::returnInput("Invalid email.", false);
        }
    ...
    public static function processPassword(string $password, string $password_v = null) {
        ...
        if (empty($password)) {
            return self::returnInput("Please enter a passwrod.", false);
        }
        ...
        if (preg_match('/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]{8,}$/', $password)) {
            ...
            return self::returnInput('Your password must have a minimum of 8 characters, at least 1 uppercase letter, 1 lowercase letter, 1 number and 1 special character', false);
        }
